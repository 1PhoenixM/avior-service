...
Local .jar file ran the client. Not web-based. Only Floodlight's API was supported. Required the download of a pre-compiled .jar specific to operating system (i.e. different download for Windows, for Linux, etc.) Included:
...
node_modules/ - Contains node.js dependencies. Probably not to be messed with. Run 'npm install <modulename>@latest' to update these.

tasks/ - Code for the Grunt utility, no Avior source code here.

views/ - Sails views (UIs) that are used for controller choice, login, plugins, 404s and anything else that affects the back-end.
...
toClient.js  - A very important file to the Avior source code. Within this module is the callback function to be executed when the data has come back from the controller. This file is run no matter what controller is being used. It makes many checks for validation of the JSON, catches some errors, then calls .parse and .normalize for the controller when it is satisfied that the JSON is valid. Finally, it calls cb(), the instantiation of Sails' function to send the JSON up to the browser. This is only done for GET requests.
